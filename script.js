// ======================
// PRELOAD (images + audio) with cache
// ======================
...
let gameStarted = false; // gate enter/space start
...
// --- Make canvas focusable & keep focus (fix F11/Space issues) ---
...
// Wrap key handlers to prevent default space/enter behavior
    ...
    setTimeout(function () {
        ...
        if (bgMusic.paused) {
            setTimeout(function () {
                ...
                focusCanvasSoon(); // ensure keys work after transition
            }, 3000);
        }
    }, 3000);
}

// Gate start via state instead of {once:true}
...
// ======================
// DEALING ANIMATION (uses preloaded images)
// ======================
const CARD_W = 120;
const CARD_H = 150;
...
function showEscapePrompt() {
    ...
    focusCanvasSoon(); // be ready to capture Space
}
...
// ======================
// GAME START (after preload)
// ======================
function startGame() {
    // initial line after preload
    ...
    // robust key listeners (window + document), no {once:true}

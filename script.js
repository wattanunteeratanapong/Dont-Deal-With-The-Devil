    ...
    return new Promise((resolve) => {
        ...
        typingInterval = setInterval(() => {
            ...
            if (activeCharSound && text.charAt(i) !== ' ') {
                try { activeCharSound.currentTime = 0; activeCharSound.play(); } catch (_) { }
            }
            i++;
            if (i >= text.length) {
                clearInterval(typingInterval);
                typingInterval = null;
                if (activeCharSound) { try { activeCharSound.pause(); activeCharSound.currentTime = 0; } catch (_) { } activeCharSound = null; }
                if (displayTime > 0) {
    ...
    setTimeout(function () {
        ...
        if (bgMusic.paused) {
            setTimeout(function () {
                ...
                document.getElementById('playPauseImage').src = './Assets/pause.png';
                const t = 5000;
                displayRunningText("Welcome to the game!", 35, t, blip);
                setTimeout(() => {
                    displayRunningText("You have already signed the contract.", 35, t, blip);
                    setTimeout(() => {
                        displayRunningText("There is no turning back now.", 35, t, blip);
                        setTimeout(() => {
                            displayRunningText("You must play until the end.", 35, t, blip);
                            setTimeout(() => {
                                displayRunningText("You have played Blackjack before, right?", 35, t, blip);
                                setTimeout(() => {
                                    displayRunningText("If you reach 100,000 points, you win.", 35, t, blip);
                                    setTimeout(() => {
                                        displayRunningText("If you have nothing to bet, you lose.", 35, t, blip);
                                        setTimeout(() => {
                                            displayRunningText("Simple, right?", 35, t, blip);
                                            setTimeout(updateBetButtons, t);
                                        }, t);
                                    }, t);
                                }, t);
                            }, t);
...
const loseVoiceLines = [
    ...
    "Huh?",
    "You sign the contract the moment you press Space.",
    "Ha ha ha...",
